...
@section('page-title', 'Edit Coupon: ' . $coupon->name)

@section('content')
<div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl shadow-lg mb-8 p-6 text-white">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold mb-2">Edit Coupon: {{ $coupon->name }}</h1>
            <p class="text-orange-100">Update coupon information and settings</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('admin.coupons.show', $coupon->id) }}" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 hover:scale-105">
                <i class="fas fa-eye mr-2"></i> View Coupon
            </a>
...
<div class="bg-white rounded-xl shadow-lg p-8">
    <form method="POST" action="{{ route('admin.coupons.update', $coupon->id) }}" enctype="multipart/form-data" id="couponForm">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="code" id="code" required
                           ...
                           placeholder="e.g., SAVE20" value="{{ old('code', $coupon->code) }}">
                    @error('code')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
                           placeholder="e.g., 20% Off Sale" value="{{ old('name', $coupon->name) }}">
                    @error('name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="value" id="value" required step="0.01" min="0"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
                               placeholder="0.00" value="{{ old('value', $coupon->value) }}">
                ...
                <div>
                    ...
                    <input type="number" name="minimum_amount" id="minimum_amount" step="0.01" min="0"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
                           placeholder="0.00" value="{{ old('minimum_amount', $coupon->minimum_amount) }}">
            ...
            <div class="mt-6">
                ...
                <input type="number" name="maximum_discount" id="maximum_discount" step="0.01" min="0"
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
                       placeholder="0.00" value="{{ old('maximum_discount', $coupon->maximum_discount) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="usage_limit" id="usage_limit" min="1"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
                           placeholder="Leave empty for unlimited" value="{{ old('usage_limit', $coupon->usage_limit) }}">
                    @error('usage_limit')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="usage_limit_per_user" id="usage_limit_per_user" min="1"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
                           placeholder="Leave empty for unlimited" value="{{ old('usage_limit_per_user', $coupon->usage_limit_per_user) }}">
        ...
        <div class="mb-8">
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div class="flex items-start">
                    ...
                    <div>
                        <h4 class="text-sm font-medium text-yellow-800">Usage Statistics</h4>
                        <div class="mt-2 text-sm text-yellow-700">
                            <p><strong>Total Usage:</strong> {{ $coupon->usages->count() }} times</p>
                            <p><strong>Unique Users:</strong> {{ $coupon->usages->pluck('user_id')->unique()->count() }} users</p>
                            @if($coupon->usage_limit)
                                <p><strong>Usage Limit:</strong> {{ $coupon->usage_limit }} ({{ round(($coupon->usages->count() / $coupon->usage_limit) * 100, 1) }}% used)</p>
                            @endif
                        </div>
                    </div>
                </div>

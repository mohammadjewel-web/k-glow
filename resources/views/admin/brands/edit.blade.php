...
<div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl shadow-lg mb-8 p-6 text-white">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold mb-2">Edit Brand</h1>
            <p class="text-orange-100">Update {{ $brand->name }} brand information</p>
        </div>
        <div class="flex space-x-3">
            ...
            <a href="{{ route('admin.brands.show', $brand->id) }}" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 hover:scale-105">
                <i class="fas fa-eye mr-2"></i> View Brand
            </a>
        </div>
    </div>
</div>
        ...
        <div class="mb-8">
            ...
            <div class="mb-4">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="w-20 h-20 bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg flex items-center justify-center shadow-md" style="display: none;">
                        <i class="fas fa-image text-gray-400"></i>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">{{ $brand->logo }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="logo" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-upload mr-1 text-orange-600"></i>{{ $brand->logo ? 'New Brand Logo' : 'Brand Logo' }}
                    </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="featured" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300">
                        <option value="0" {{ old('featured', $brand->featured ? '1' : '0') == '0' ? 'selected' : '' }}>Regular</option>
                        <option value="1" {{ old('featured', $brand->featured ? '1' : '0') == '1' ? 'selected' : '' }}>Featured</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="nav" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300">
                        <option value="0" {{ old('nav', $brand->nav ? '1' : '0') == '0' ? 'selected' : '' }}>Hidden</option>
                        <option value="1" {{ old('nav', $brand->nav ? '1' : '0') == '1' ? 'selected' : '' }}>Show in Nav</option>
                    </select>
                </div>
            </div>
        </div>

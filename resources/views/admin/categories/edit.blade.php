...
<div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl shadow-lg mb-8 p-6 text-white">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold mb-2">Edit Category</h1>
            <p class="text-orange-100">Update category information</p>
        </div>
        <div class="flex space-x-3">
            ...
            <a href="{{ route('admin.categories.show', $category->id) }}" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 hover:scale-105">
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description" rows="4" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
                ...
                @if($category->image)
                    <div class="mb-3">
                        <img src="{{ asset('admin-assets/categories/' . $category->image) }}" 
                             class="w-20 h-20 object-cover rounded-lg border border-gray-300" alt="Current Image">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div>
                ...
                <select id="featured" name="featured" 
                    ...
                    <option value="0" {{ old('featured', $category->featured ? '1' : '0') == '0' ? 'selected' : '' }}>Regular</option>
                    <option value="1" {{ old('featured', $category->featured ? '1' : '0') == '1' ? 'selected' : '' }}>Featured</option>
                </select>
                @error('featured')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="nav" name="nav" 
                    ...
                    <option value="0" {{ old('nav', $category->nav ? '1' : '0') == '0' ? 'selected' : '' }}>Hidden</option>
                    <option value="1" {{ old('nav', $category->nav ? '1' : '0') == '1' ? 'selected' : '' }}>Show in Nav</option>
                </select>
                @error('nav')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="mb-6">
            ...
            <textarea id="meta_description" name="meta_description" rows="3" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 @error('meta_description') border-red-500 @enderror">{{ old('meta_description', $category->meta_description) }}</textarea>
